//--- Aura Script -----------------------------------------------------------
// Edern, the blacksmith of Bangor
//--- Description -----------------------------------------------------------
// Weapon Forger
//---------------------------------------------------------------------------

public class EdernBaseScript : NpcScript
{
	public override void Load()
	{
		SetName("_edern");
		SetRace(10002);
		SetBody(height: 1.3f, upper: 2.0f, lower: 1.4f);
		SetFace(skinColor: 25, eyeType: 9, eyeColor: 38, mouthType: 2);
		SetStand("human/male/anim/male_natural_stand_npc_edern");
		SetLocation(31, 10972, 13373, 1);

		EquipItem(Pocket.Face, 4904, 0x00C6C561, 0x00E1C210, 0x00E8A14A);
		EquipItem(Pocket.Hair, 4028, 0x002E303F, 0x002E303F, 0x002E303F);
		EquipItem(Pocket.Armor, 15039, 0x001F2340, 0x00988486, 0x009E9FAC);
		EquipItem(Pocket.Shoe, 17504, 0x0077564A, 0x00F2A03A, 0x008A243D);
		EquipItem(Pocket.RightHand1, 40024, 0x00FACB5F, 0x004F3C26, 0x00FAB052);

		AddPhrase("Testing Script Speech Bubble");
		...
		switch (await Select())
		{
			...
			case "@repair":
				...
				while(true)
				{
					...
					var result = Repair(repair, 97, "/smith_repairable/");
					if(!result.HadGold)
					{
						RndMsg(
